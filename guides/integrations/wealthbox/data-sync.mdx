...
<AccordionGroup>
  ...
  <Accordion title="System tracking fields">
    ...
    <ParamField body="SourceMeta" type="object">
      Contains reference IDs for maintaining data relationships:
      - `SourceEmailId` - Primary email reference ID
      - `SourcePhoneId` - Primary phone reference ID  
      - `SourceAddressId` - Primary address reference ID
      - `SourceVisibleToId` - Visibility permissions reference
    </ParamField>
  </Accordion>
  ...
  <Accordion title="Tags">
    CurrentClient automatically creates reads the tags from your Wealthbox contact and creates a matching tag on the CurrentClient contact.
  </Accordion>
...
<Note>
**Conversation summaries:** CurrentClient can automatically create conversation summaries that get added as notes to your Wealthbox client records.
</Note>
